...
app_name = Rails.configuration.gcm_app
app = Rpush::Gcm::App.where(name: app_name)
if app.nil?
  app = Rpush::Gcm::App.new
  app.name = app_name
  app.auth_key = Rails.configuration.gcm_api_key
  app.save!
end
if defined?(Rails)
    ...
    module Dispatcher
      class Http
        def initialize(app, delivery_class, _options = {})
          ...
          proxy = Rails.configuration.gcm_proxy
          @http = Net::HTTP::Persistent.new('rpush', proxy)
        end

        def dispatch(payload)
          p 'dispatching payload DUDE LOOK THIS'
